...
var svg_bar = d3
	.select('#chart-nilai-tertinggi')
	.append('svg')
	.attr('width', size_bar * 2)
	.attr('height', size_bar + 20)
	.attr('class', 'bar');

var chart_bar = svg_bar.append('g').attr('transform', 'translate(' + (margin + 25) + ',' + margin + ')');
...
var yScale = d3.scaleLinear().range([bar_height, 0]).domain([0, 840]);
...
chart_bar.append('g').call(d3.axisLeft(yScale));

chart_bar.append('g').attr('class', 'grid').call(makeYLines().tickSize(-bar_width, 0, 0).tickFormat(''));

var barGroups = chart_bar.selectAll().data(data_tertinggi).enter().append('g');

barGroups
	.append('rect')
	.attr('class', 'bar')
	.attr('x', function (g) {
		return xScale(g.nama);
	})
...
var barGroups = chart_bar.selectAll().data(data_terendah).enter().append('g');
